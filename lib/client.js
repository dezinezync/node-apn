...
module.exports = function (dependencies) {
  ...
  Client.prototype.write = function write (notification, device, count) {
    return this.getStream().then( stream => {
      let tokenGeneration, status, responseData = "", responseHeaders = {};
      ...
      stream.on("headers", headers => {
        ...
        responseHeaders = Object.assign({}, responseHeaders, headers);
      });
        ...
        stream.on("end", () => {
          if (status === "200") {
            if (this.config.production) {
              ...
              let uniqueID = responseHeaders["apns-unique-id"];
              resolve({ device, uniqueID });
            }
